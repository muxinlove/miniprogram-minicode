// https://github.com/jinxuanzheng01/wx-rss.js
const Rss = {
  // 缓存事件对象
  _storeObj: {},
   ...
   */
  named(name) {
    if (~name.indexOf("coms")) {
      const routes = getCurrentPages();
      const route = routes[routes.length - 1];
      const NodeId = route.__wxExparserNodeId__;
      return `${NodeId}/${name}`;
    }
    return name;
  },
  ...
  _bind(name, callback, context, type) {
    ...
    // 变量声明
    const { _storeObj } = this; // 缓存全部对象
    let _currentObj = _storeObj[name]; // 当前对象
    let _count = 0; // 计数
    ...
    // 分类判定
    const sort = _storeObj[name].length;
    let juge = true;

    // 判断是否含有该属性
    for (const i in _storeObj[name]) {
      !!_storeObj[name][i].context && _storeObj[name][i].context === context
        ? (juge = false)
        : "";
    }
